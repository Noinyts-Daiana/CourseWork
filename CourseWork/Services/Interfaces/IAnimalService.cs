...
public interface IAnimalService
{
    Task<IEnumerable<AnimalDto>> GetAllAnimalsAsync(int pageNumber, int pageSize, string? searchTerm,
        List<int>? charIds, int? speciesId, int? breedId, int? sex, bool? isAdopted);
    ...
    Task<int> GetAnimalsCountAsync(string? searchTerm, List<int>? charIds, int? speciesId, int? breedId, int? sex, bool? isAdopted);
}